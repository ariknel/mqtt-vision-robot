...
| 4 Jun 2026 | First line-following calibration session. Test track: electrical tape on floor (~18 mm wide). Robot could detect the line but repeatedly drove off — speed too high for the IR reaction time, correction turns overshot. Speed and turn ratios adjusted iteratively during session. |
| 5 Jun 2026 | Second calibration session. Robot successfully followed a 90-degree curved section of tape at low speed. Motors occasionally failed to start from standstill — stall at low PWM duty; required a physical nudge to break static friction. Minimum startup speed noted as a tuning target. |
| 6 Jun 2026 | Third calibration session. Line following stable enough for consistent laps on the electrical tape track — robot stays on the line through gentle curves without intervention. Motor stall issue reduced by raising the minimum PWM floor. Behaviour considered stable enough for initial validation; further testing needed on a thicker line and sharper turns before calling calibration complete. |
